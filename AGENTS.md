...
> **STATUS — PRODUCT DECISION (2026-09-12): POSTPONED, OUT OF CURRENT SCOPE.**
> The Admin Dashboard described in this section (product operations, users, usage logs,
> AI/billing operations views, audit log UI, `/admin` routes) will **not** be implemented
> in the current plan. It is cancelled for now and will be reconsidered later — do not
> build it in any current implementation step. The underlying data foundations remain
> part of the architecture as designed (ai_usage ledger §14, audit events, analytics
> events §42) so the dashboard can be added later without rework.
...
12. Admin Dashboard + analytics + audit logs — POSTPONED (2026-09-12 product decision, see §41 note); data foundations only
...
> **STATUS — PRODUCT DECISION (2026-09-12): POSTPONED, OUT OF CURRENT SCOPE** (see §41 note). Kept for reference; nothing here is being built in the current plan.
...
> **STATUS — PRODUCT DECISION (2026-09-12): POSTPONED, OUT OF CURRENT SCOPE.**
> Same decision as the §41 note above: this admin dashboard (users, subscriptions,
> AI usage, logs, audit, system health views) is **not** part of the current
> implementation plan and will be reconsidered later. Do not build it now. The
> data layers it would read (ai_usage ledger, audit events, product analytics)
> remain in the architecture as designed.
