...
| D6  | EntitlementService = static guest config + budget logic in the worker: `GUEST_ENTITLEMENTS`, `getEntitlements(identity)`, pure `budgetOutcome(limits, used, feature)`, `buildGuestSessionResponse(...)` in `services/entitlements.ts`; D1 usage persistence (`getGuestUsage`, `recordGuestUsage`) lives in `services/guestSessions.ts` (DB layering). Plans table (§16) arrives with billing; feature routes compose both layers (§34). | §17 centralization, §11.2 product-usage vs billable-usage separation; "configuration data, not frontend hardcoded rules" (§5.1). Shape reconciled post-review (pure decision vs D1 I/O split). |
