...
export function verificationEn({ url, email }: TemplateParams): TemplateContent {
  const subject = "Verify your LearWizAI email address";
  const heading = "Confirm your email";
  const paragraphs = [
    email ? `Welcome to LearWizAI, ${email}!` : "Welcome to LearWizAI!",
  ...
  return {
    ...
    html: htmlShell(
      ...
      "Verify email",
      url,
      "You received this email because an account was created on LearWizAI.",
    ),
...
export function welcomeEn({ url }: TemplateParams): TemplateContent {
  const subject = "Welcome to LearWizAI";
...
export function passwordResetEn({ url }: TemplateParams): TemplateContent {
  const subject = "Reset your LearWizAI password";
  ...
  return {
    ...
    html: htmlShell(
      ...
      url,
      "LearWizAI — security notification.",
    ),
    text: `${paragraphs.join("\n\n")}\n\n${url}`,
  };
}
